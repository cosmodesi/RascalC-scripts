...
if args.ntasks:
    ...
    sys.exit(0)
if args.list_cases:
    print_case_summary(args.campaign, compmd=args.compmd, version=args.version, phase="pre", default_n_loops=args.default_n_loops)
    sys.exit(0)
if args.id is None:
...
try:
    n_loops = get_n_loops("pre", tlabels[0], z_range, reg, default=args.default_n_loops, allow_missing=args.test)
except KeyError as exc:
    parser.error(str(exc))
if n_loops is None:
    print("No tuned n_loops for this case; continuing input test with a placeholder value.")
    n_loops = nthread
...
outdir_base = os.path.join(verspec, version, compmd, conf, "_".join(tlabels + [reg]) + f"_z{z_min:g}-{z_max:g}")
