...
for tlabels, z_range in zip(tracers, zs):
    ...
    reg_pycorr_names = []
    for reg in regs:
        outdir = os.path.join("outdirs", verspec, version, conf, "_".join([combined_tracer,reg]) + f"_z{z_min}-{z_max}") # output file directory
        ...
        cov_dir = f"cov_txt/{verspec}/{version}/{conf}/{combined_tracer}_as_multi"
        ...
        tmp_pycorr_names = [f.filepath for f in fm.select(id = 'correlation_y3', **xi_setup)] # get pycorr file paths for the combined tracer from file manager
    ...
    if len(reg_pycorr_names) == len(regs): # if we have pycorr files for all regions
        if len(reg_results) == len(regs): # if we have RascalC results for all regions
            ...
            my_make(cov_name, reg_results, lambda: combine_covs_legendre_multi_to_cat(*reg_results, *reg_pycorr_names, cov_name, max_l, r_step = r_step, skip_r_bins = skip_r_bins, print_function = print_and_log))
        ...
        if jackknife and len(reg_results_jack) == len(regs): # if jackknife and we have RascalC jack results for all regions
            ...
            # Make a combined tracer covariance by collapsing from multi-tracer
            cov_name_rescaled = f"{cov_dir}/xi" + xilabel + "_" + "_".join([combined_tracer, reg_comb]) + f"_z{z_min}-{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC.txt" # combined cov name
            my_make(cov_name_rescaled, reg_results_jack, lambda: combine_covs_legendre_multi_to_cat(*reg_results_jack, *reg_pycorr_names, cov_name_rescaled, max_l, r_step = r_step, skip_r_bins = skip_r_bins, print_function = print_and_log))
