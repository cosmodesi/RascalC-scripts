...
parser = argparse.ArgumentParser(description = "Main RascalC computation script for DESI Y3 post-recon combined tracers as multi-tracer")
...
if combined_tracer == 'FullCombined':
    recon_spec = 'recon_sm15_IFFT_recsym_z0.8-1.1' # recon specifier string, hard-coded for the full combined tracer
    catalog_dir = f"/dvs_ro/cfs/cdirs/desi/users/sandersn/DA2/{verspec}/{version}/{conf_alt}/full/{recon_spec}/" # directory for catalogs
else:
    common_setup = {"region": reg, "version": version} # grid_cosmo will be specified with the xi_setup or recon_setup
    ...
    recon_setup = desi_y3_file_manager.get_baseline_recon_setup(combined_tracer, z_range)
...
outdir_base = os.path.join(verspec, version, conf, recon_spec, "_".join([combined_tracer, reg]) + f"_z{z_min}-{z_max}")
...
if combined_tracer == 'FullCombined': # for the full combined tracer, we don't have the files in desi_y3_file_manager, so we hard-code the directory and filenames
    ...
    random_filenames = [f.filepath for f in fm.select(id='catalog_randoms_recon_y3', tracer=combined_tracer, iran=range(nrandoms), **(common_setup | recon_setup))]
print("Random filenames:", random_filenames)
if njack:
    if combined_tracer == 'FullCombined':
        data_ref_filename = f"{catalog_dir}/{combined_tracer}_{reg}_clustering.dat.fits"
    else:
        data_ref_filename = fm.select(id='catalog_data_recon_y3', tracer=combined_tracer, **(common_setup | recon_setup))[0].filepath
