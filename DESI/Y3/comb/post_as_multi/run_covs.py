...
parser = argparse.ArgumentParser(description = "Main RascalC computation script for DESI Y3 post-recon combined tracer as multi-tracer")
...
if njack:
    data_ref_filename = fm.select(id='catalog_data_recon_y3', tracer=combined_tracer, **(common_setup | recon_setup))[0].filepath # only for jackknife reference, could be used for determining the number of galaxies but not in this case
