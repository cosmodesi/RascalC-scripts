...
for tracer, z_range, nrandoms, recon_zrange in zip(tracers, zs, ns_randoms, recon_zranges):
    tlabels = [tracer]
    z_min, z_max = z_range
    reg_results = []
    # recon options
    ...
    reg_pycorr_names = [f"/dvs_ro/cfs/cdirs/desi/users/sandersn/DA2/{verspec}/{version}/{conf}/full_fixed/{recon_spec}/xi/smu/allcounts_{tracer}_{reg}_{z_min}_{z_max}_default_FKP_lin_njack{njack}_nran{nrandoms}_split20.npy" for reg in regs]
