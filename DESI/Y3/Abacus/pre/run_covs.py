...
parser = argparse.ArgumentParser(description = "Main RascalC computation script for DESI Y3 Abacus pre-recon single-tracer")
...
# need 2 * 13 = 26 jobs in this array
...
all_mocks_setup = {"region": reg, "version": version, "fa": fa, "grid_cosmo": None}
...
input_xi_filenames = [[f.filepath for f in fm.select(id = 'correlation_abacus_y3', tracer = corlabel, njack = 0, **(all_mocks_setup | xi_setup))] for corlabel in corlabels]
print("input xi filenames:", input_xi_filenames)
counts_jack_filenames = [[f.filepath for f in fm.select(id = 'correlation_abacus_y3', tracer = corlabel, njack = njack, **(single_mock_setup | xi_setup))] for corlabel in corlabels]
print("counts/jack filenames:", counts_jack_filenames)

# Filenames for randoms and galaxy catalogs
random_filenames = [[f.filepath for f in fm.select(id = 'catalog_randoms_abacus_y3', tracer = tlabel, iran = range(nrandoms), **single_mock_setup)] for tlabel in tlabels]
print("Random filenames:", random_filenames)
if njack:
    data_ref_filenames = [fm.select(id = 'catalog_data_abacus_y3', tracer = tlabel, **single_mock_setup)[0].filepath for tlabel in tlabels] # only for jackknife reference, could be used for determining the number of galaxies but not in this case
