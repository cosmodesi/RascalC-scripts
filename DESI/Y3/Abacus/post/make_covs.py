...
for tracer, z_range in zip(tracers, zs):
    ...
    for reg in regs:
        outdir = os.path.join("outdirs", version, recon_spec, "_".join(tlabels + [reg]) + f"_z{z_min}-{z_max}") # output file directory
        ...
        cov_dir = f"cov_txt/{version}/{fa}/{recon_spec}"
    ...
    reg_pycorr_names = [f.filepath for f in fm.select(id = 'correlation_recon_abacus_y3', **xi_setup)]
