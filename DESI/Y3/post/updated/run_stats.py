...
module unload desi-clustering # temporarily to resolve the nz filename bug, need to install a more up-to-date version of desi-clustering in user space
...
def run_stats(tracer='LRG', project='', version='holi-v3-altmtl', onthefly=None, stats_dir=Path(os.getenv('SCRATCH')) / 'measurements', stats=['mesh2_spectrum'], weight='default-FKP', analysis='full_shape', regions=['NGC','SGC'], ibatch=None, postprocess=None, zranges=None, do_jackknife=False, **kwargs):
    ...
    for region in regions:
        ...
        options['recon_particle2_correlation'].update({'battrs': dict(s=np.linspace(0., 180., 181), mu=(np.linspace(-1., 1., 201), 'midpoint'))}) # force the computation of large-scale correlation functions against the default for protected
...
def postprocess_stats(tracer='LRG', analysis='full_shape', project='', version='holi-v3-altmtl', onthefly=None, stats_dir=Path(os.getenv('SCRATCH')) / 'measurements', stats=['mesh2_spectrum'], weight='default-FKP', postprocess=['combine_regions'], zranges=None, do_jackknife=False, **kwargs):
    ...
    options = dict(catalog=dict(version=version, tracer=tracer, zrange=zranges, weight=weight), combine_regions={'stats': stats}, mesh2_spectrum={'cut': True, 'auw': True}, window_mesh2_spectrum={'cut': True}, recon_particle2_correlation={'jackknife': {'nsplits': 60}} if do_jackknife else {})
