...
for tracer, (z_min, z_max) in zip(tracers, zs):
    ...
    for reg in regs:
        ...
        cov_name = f"cov_txt/{version}/{conf}/{recon_spec}/xi" + xilabel + "_" + "_".join(tlabels + [reg]) + f"_z{z_min}-{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_Gaussian.txt"
        ...
        if jackknife:
            ...
            cov_name_jack = f"cov_txt/{version}/{conf}/{recon_spec}/xi" + xilabel + "_" + "_".join(tlabels + [reg]) + f"_z{z_min}-{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC.txt"
            # Individual cov file depends on RascalC results
    ...
    if len(reg_pycorr_names) == len(regs): # if we have pycorr files for all regions
        if len(reg_results) == len(regs): # if we have RascalC results for all regions
            ...
            cov_name = f"cov_txt/{version}/{conf}/{recon_spec}/xi" + xilabel + "_" + "_".join(tlabels + [reg_comb]) + f"_z{z_min}-{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_Gaussian.txt" # combined cov name
