...
for tracer, z_range in zip(tracers, zs):
    ...
    for reg in regs:
        if make_mock_cov:
            ...
            imocks = np.loadtxt(f"{version_dark}_dark-time_imocks_for_covariance.txt", dtype=int) # list of mocks to use for covariance
            xi_filenames = [get_stats_fn(imock=imock, **stats_kws) for imock in imocks] # no jackknife, all mocks from the list
