# Post-process RascalC outputs and export covariance matrices for high-redshift QSO auto-correlation.
...
for tracer, z_range in zip(tracers, zs):
    ...
    z_min, z_max = z_range
    reg_results = []
    # get options automatically; njack must match run_covs.py so combine_regions finds the right 2pt files
    xi_setup = desi_y3_file_manager.get_baseline_2pt_setup(tlabels[0], z_range)
    xi_setup.update({"version": version, "tracer": tracer, "region": regs, "zrange": z_range, "cut": None, "njack": njack})
