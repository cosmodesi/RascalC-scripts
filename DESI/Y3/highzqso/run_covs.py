### Python script for running RascalC on high-redshift QSO auto-correlation (DESI DR2, pre-recon).
import sys, os
import numpy as np
...
parser = argparse.ArgumentParser(description = "RascalC computation for high-redshift QSO auto-correlation (DESI Y3 pre-recon)")
parser.add_argument("id", type = int, help = "SLURM array task id: even = SGC, odd = NGC")
...
# --- High-z QSO analysis configuration ---
...
# Set DESI CFS before creating the file manager (override via DESICFS in the shell)
os.environ.setdefault("DESICFS", "/dvs_ro/cfs/cdirs/desi")

fm = desi_y3_file_manager.get_data_file_manager(conf, verspec)

id = args.id # SLURM array task id
