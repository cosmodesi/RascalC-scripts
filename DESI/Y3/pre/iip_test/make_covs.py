...
for tracer, z_range in zip(tracers, zs):
    ...
    reg_counts_names = [get_stats_fn(version=version, tracer=tracer, region=reg, zrange=z_range, stats_dir=stats_dir, project='protected/base', kind='particle2_correlation', weight='default-FKP-bitwise-iip', jackknife=dict(nsplits=njack)) for reg in regs] # don't need jackknife but jackknife-less counts not present (for now)
