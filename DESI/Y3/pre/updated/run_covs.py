...
parser = argparse.ArgumentParser(description="Main RascalC computation script for DESI Y3 pre-recon single-tracer (using desi-clustering pipeline instead of desi-y3-kp)")
...
allcounts_filenames = [get_stats_fn(version=version, tracer=corlabel, region=reg, zrange=z_range, stats_dir='.', project='protected/base', kind='particle2_correlation', weight='default-FKP', jackknife=dict(nsplits=njack)) for corlabel in corlabels]
