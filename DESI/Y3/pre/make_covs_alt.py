# This script generates older covariances for narrower z bins in alternative separation ranges and multipoles
...
r_step = rmax // nbin
rmin_real = r_step * skip_r_bins[0]
rmax_real = r_step * (nbin - skip_r_bins[1])
max_l_real = max_l - 2 * skip_l

xilabel = "".join([str(i) for i in range(0, max_l_real + 1, 2)])
...
tracers = ['BGS_BRIGHT-21.35', 'BGS_BRIGHT-20.2']
zs = [(0.25, 0.4), (0.1, 0.25)]
...
for tracer, z_range in zip(tracers, zs):
    ...
    for reg in regs:
        ...
        cov_name = f"{cov_dir}/xi" + xilabel + "_" + "_".join(tlabels + [reg]) + f"_z{z_min}-{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax_real}_cov_RascalC_Gaussian.txt"
        ...
        my_make(cov_name, [results_name], lambda: export_cov_legendre(results_name, max_l_real, cov_name))
        # Recipe: export cov
        ...
        if jackknife:
            ...
            cov_name_jack = f"{cov_dir}/xi" + xilabel + "_" + "_".join(tlabels + [reg]) + f"_z{z_min}-{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax_real}_cov_RascalC.txt"
            # Individual cov file depends on RascalC results
            my_make(cov_name_jack, [results_name_jack], lambda: export_cov_legendre(results_name_jack, max_l_real, cov_name_jack))
    ...
    if len(reg_pycorr_names) == len(regs): # if we have pycorr files for all regions
        if len(reg_results) == len(regs): # if we have RascalC results for all regions
            ...
            cov_name = f"{cov_dir}/xi" + xilabel + "_" + "_".join(tlabels + [reg_comb]) + f"_z{z_min}-{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax_real}_cov_RascalC_Gaussian.txt" # combined cov name

            # Comb cov depends on the region RascalC results
            my_make(cov_name, reg_results, lambda: combine_covs_legendre(*reg_results, *reg_pycorr_names, cov_name, max_l_real, r_step = r_step, skip_r_bins = skip_r_bins, print_function = print_and_log))
        ...
        if jackknife and len(reg_results_jack) == len(regs): # if jackknife and we have RascalC jack results for all regions
            ...
            cov_name_jack = f"{cov_dir}/xi" + xilabel + "_" + "_".join(tlabels + [reg_comb]) + f"_z{z_min}-{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax_real}_cov_RascalC.txt" # combined cov name

            # Comb cov depends on the region RascalC results
            my_make(cov_name_jack, reg_results_jack, lambda: combine_covs_legendre(*reg_results_jack, *reg_pycorr_names, cov_name_jack, max_l_real, r_step = r_step, skip_r_bins = skip_r_bins, print_function = print_and_log))
            # Recipe: run combine covs
