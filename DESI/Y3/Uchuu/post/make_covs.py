...
tracers = ['LRG'] * 3 + ['ELG_LOPnotqso'] * 2 + ['QSO']
zs = [(0.4, 0.6), (0.6, 0.8), (0.8, 1.1), (0.8, 1.1), (1.1, 1.6), (0.8, 2.1)]
...
for tracer, z_range in zip(tracers, zs):
    ...
    for reg in regs:
        ...
        cov_dir = f"cov_txt/{version}/mock{mock_id}/recsym_IFFT"
    ...
    # obtain the counts names (use jackknife pair counts since non-jackknife don't exist at bao/base)
    reg_counts_names = [get_stats_fn(version=version, imock=mock_id, tracer=tracer, region=reg, zrange=z_range, stats_dir=stats_dir, project='bao/base', kind='recon_particle2_correlation', weight='default-FKP', jackknife=dict(nsplits=60)) for reg in regs]
