...
for tracer, (z_min, z_max) in zip(tracers, zs):
    ...
    for mock_id in mock_ids:
        ...
        for reg in regs:
            ...
            if jackknife:
                cov_name_jack = f"cov_txt/mock{mock_id}/xi" + xilabel + "_" + "_".join(tlabels + [rectype, f"sm{sm}", reg]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_rescaled.txt"
            ...
            if mock_post_processing:
                cov_name_mocks = f"cov_txt/mock{mock_id}/xi" + xilabel + "_" + "_".join(tlabels + [rectype, f"sm{sm}", reg]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_rescaled_mocks.txt"
        ...
        if len(reg_pycorr_names) == len(regs): # if we have pycorr files for all regions
            ...
            if jackknife and len(reg_results_jack) == len(regs): # if jackknife and we have RascalC jack results for all regions
                ...
                cov_name_jack = f"cov_txt/mock{mock_id}/xi" + xilabel + "_" + "_".join(tlabels + [rectype, f"sm{sm}", reg_comb]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_rescaled.txt" # combined cov name
            ...
            if mock_post_processing and len(reg_results_mocks) == len(regs): # if mock post-processing and we have RascalC mocks results for all regions
                # Combined rescaled cov
                cov_name_mocks = f"cov_txt/mock{mock_id}/xi" + xilabel + "_" + "_".join(tlabels + [rectype, f"sm{sm}", reg_comb]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_rescaled_mocks.txt" # combined cov name
