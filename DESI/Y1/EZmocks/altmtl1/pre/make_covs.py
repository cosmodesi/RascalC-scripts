...
input_dir = f"/dvs_ro/cfs/cdirs/desi/survey/catalogs/Y1/mocks/SecondGenMocks/ALTMTL_EZmock/altmtl{mock_id}/mock{mock_id}/LSScats/"
...
for tracer, (z_min, z_max), nrandoms in zip(tracers, zs, ns_randoms):
    tlabels = [tracer]
    reg_results, reg_pycorr_names = [], []
    if jackknife or mocks: reg_results_rescaled = []
    for reg in regs:
        outdir = os.path.join("outdirs", f"mock{mock_id}", "_".join(tlabels + [reg]) + f"_z{z_min}-{z_max}") # output file directory
        ...
        cov_name = f"cov_txt/altmtl{mock_id}/xi" + xilabel + "_" + "_".join(tlabels + [reg]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_Gaussian.txt"
        this_reg_pycorr_filenames = []
        reg_pycorr_names.append(input_dir + f"xi/smu/allcounts_{tracer}_{reg}_z{z_min}-{z_max}_default_FKP_lin_nran{nrandoms}_njack{njack}_split{split_above}.npy")
        ...
        if jackknife or mocks:
            cov_name_rescaled = f"cov_txt/altmtl{mock_id}/xi" + xilabel + "_" + "_".join(tlabels + [reg]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_rescaled.txt"
    ...
    if len(reg_pycorr_names) == len(regs): # if we have pycorr files for all regions
        if len(reg_results) == len(regs): # if we have RascalC results for all regions
            ...
            cov_name = f"cov_txt/altmtl{mock_id}/xi" + xilabel + "_" + "_".join(tlabels + [reg_comb]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_Gaussian.txt" # combined cov name
        ...
        if (jackknife or mocks) and len(reg_results_rescaled) == len(regs): # if jackknife and we have RascalC jack results for all regions
            ...
            cov_name_rescaled = f"cov_txt/altmtl{mock_id}/xi" + xilabel + "_" + "_".join(tlabels + [reg_comb]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_rescaled.txt" # combined cov name
