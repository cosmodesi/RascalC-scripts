...
outdir_base = "_".join(tlabels + [reg]) + f"_z{z_min}-{z_max}"
...
split_above = 20
pycorr_filenames = [[input_dir + f"mock{i+1}/xi/smu/allcounts_{corlabel}_{reg}_z{z_min}-{z_max}_default_FKP_lin_nran{nrandoms}_njack{njack}_split{split_above}.npy" for i in range(1000)] for corlabel in corlabels]

# Filenames for randoms and galaxy catalogs
random_filenames = [[input_dir + f"merged/{tlabel}_{reg}_{i}_clustering.ran.fits" for i in range(nrandoms)] for tlabel in tlabels]
data_ref_filenames = [input_dir + f"merged/{tlabel}_{reg}_clustering.dat.fits" for tlabel in tlabels] # only for jackknife reference, could be used for determining the number of galaxies but not in this case
