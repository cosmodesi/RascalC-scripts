...
input_dir = f"/pscratch/sd/e/epaillas/recon_weights/growth_bias_weights/{version}/{conf}/"

# Output and temporary directories

outdir_base = os.path.join(version, conf, f"recon_{rectype}", "_".join(tlabels + [reg]) + f"_z{z_min}-{z_max}")
...
pycorr_filenames = [[input_dir + f"smu/allcounts_{corlabel}_{rectype}_{reg}_{z_min}_{z_max}_default_FKP_lin_njack{njack}_nran{nrandoms}_split20.npy"] for corlabel in corlabels]
