...
for tracer, (z_min, z_max) in zip(tracers, zs):
    ...
    for reg in regs:
        outdir = os.path.join("outdirs", version, conf, f"recon_{rectype}", "_".join(tlabels + [reg]) + f"_z{z_min}-{z_max}") # output file directory
        ...
        cov_name = f"cov_txt/{version}/{conf}/xi" + xilabel + "_" + "_".join(tlabels + [rectype, reg]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_Gaussian.txt"
        ...
        if jackknife:
            ...
            cov_name_jack = f"cov_txt/{version}/{conf}/xi" + xilabel + "_" + "_".join(tlabels + [rectype, reg]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_rescaled.txt"
            # Individual cov file depends on RascalC results
    ...
    reg_pycorr_names = [f"/pscratch/sd/e/epaillas/recon_weights/growth_bias_weights/{version}/{conf}/smu/allcounts_{tracer}_{rectype}_{reg}_{z_min}_{z_max}_default_FKP_lin_njack{njack}_nran{nrandoms}_split20.npy" for reg in regs]
    ...
    if len(reg_pycorr_names) == len(regs): # if we have pycorr files for all regions
        if len(reg_results) == len(regs): # if we have RascalC results for all regions
            ...
            cov_name = f"cov_txt/{version}/{conf}/xi" + xilabel + "_" + "_".join(tlabels + [rectype, reg_comb]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_Gaussian.txt" # combined cov name
        ...
        if jackknife and len(reg_results_jack) == len(regs): # if jackknife and we have RascalC jack results for all regions
            ...
            cov_name_jack = f"cov_txt/{version}/{conf}/xi" + xilabel + "_" + "_".join(tlabels + [rectype, reg_comb]) + f"_{z_min}_{z_max}_default_FKP_lin{r_step}_s{rmin_real}-{rmax}_cov_RascalC_rescaled.txt" # combined cov name
