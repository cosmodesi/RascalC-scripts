...
n_randoms_per_data = 12 # how many times the number of data points per file; note that QSO are sparser than other tracers
...
n_loops = 2048 # number of integration loops per filename; may need to be adjusted with the number of random points
...
pycorr_filenames = [[f"/global/cfs/cdirs/desi/cosmosim/AbacusHOD_mocks/varietyHOD/CubicBox/{corlabel}/z{z_ref:.3f}/jmena/Xi/Pre/pycorr_format/Xi_AbacusSummit_base_c000_ph{i:03d}_HOD{hod_label}.npy" for i in range(25)] for corlabel in corlabels]
...
for t in range(len(tlabels)):
    # number of random points to make
    n_randoms_t = int(ndata[t] * n_randoms_per_data)
    # unit weights - make sure it is consistent with the counts!
    randoms_weights[t] = np.ones(n_randoms_t)
    # generate random positions between 0 and periodic_boxsize
    randoms_positions[t] = np.random.rand(n_randoms_t, 3) * periodic_boxsize
