...
DESI Y1 (DR1) data runs rely on [cosmodesi/desi-y1-kp](https://github.com/cosmodesi/desi-y1-kp) installed to retrieve the filenames.
(Earlier) DESI Y3 (DR2) data runs similarly rely on [cosmodesi/desi-y3-kp](https://github.com/cosmodesi/desi-y3-kp).
...
Later/updated DESI Y3 (DR2) runs rely on [cosmodesi/desi-clustering](https://github.com/cosmodesi/desi-clustering); this repository is already public.
...
sbatch run_covs.sh
```
then (ideally after all jobs in the resulting array finish, to make things easier),
```
python make_covs.py
```